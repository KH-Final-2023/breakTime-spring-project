...
public class ChatServiceImpl implements ChatService {
	...
	@Override
	public List<ChatMessage> joinBuChatRoom(ChatRoomJoinBu join) {
		
		// 현재 회원이 해당 채팅방에 참여하고 있는지 확인
		int result = dao.joinCheckBu(join); // CHAT_ROOM_JOIN에 현재 유저+채팅방번호로 들어간 데이터가 있는지 확인
		
		if(result == 0) { // 미참여시 추가
			dao.joinBuChatRoom(join);
		}
	...
	public int insertMessageBu(ChatMessageBu chatMessageBu) {
		...
		return dao.insertMessageBu(chatMessageBu);
	}
}
