	...
	public int insertBusiness(Business inputBusiness, HttpServletRequest req) {
		...
		//파일 업로드(우리 서버에)
				...
				if(!f.isEmpty()) {
					...
					inputBusiness.setChangeName(changeName);
					//사이즈, 타입
					System.out.println("===============");
					...
					//파일을 톰캣에 저장
					String path = req.getServletContext().getRealPath("/resources/uploadFiles/buMainImg/");
					//파일을 서버에 저장
...
//					File file = new File("D:/uploadForSpring/999prj/profile/"+ changeName);
					...
					//db에 insert
					businessDao.uploadProfile(inputBusiness);
				}
